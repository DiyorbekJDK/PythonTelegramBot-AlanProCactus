...
def start(message):
    ...
    decription = types.KeyboardButton('English language')
    photo_atom = types.KeyboardButton('Русский язык')
    video = types.KeyboardButton("O'zbek tili")
    shorts = types.KeyboardButton('Қазақ тілі')
    last_video = types.KeyboardButton('тоҷикӣ')
    markup.add(photo_atom, shorts, video, last_video, decription)
    ...
    def getMsg(message):
        if message.text == 'English language':
            ...
            def podpiska(message):
                ...
                markup.add(
                    types.InlineKeyboardButton("Are you subscribed for to the YouTube channel?", url=youTubeChannel))
                markup.add(
        ...
        if message.text == 'Русский язык':
            messs = f"Привет, <b>{message.from_user.first_name}</b>! Я телеграм бот Алана Про кактуса. Я могу дать тебе информацию об Канале Алан Про кактус. Все что нужно в меню\n/menu "
